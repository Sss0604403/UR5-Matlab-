...
a_t = zeros(length(t), joints_dof);      %关节加速度状态
...
for i = 1:joints_dof    %遍历每个关节角
    ...
    a0 = q0_i;
    a1 = 0;
    a2 = 3 * (qf_i - q0_i) / (t_all^2);
    a3 = -2 * (qf_i - q0_i) / (t_all^3);

    for j = 1:length(t)
        t_j = t(j);
        q_t(j, i) = a0 + a1 * t_j + a2 * (t_j^2) + a3 * (t_j^3);   %角度公式
        v_t(j, i) = a1 + 2 * a2 * t_j + 3 * a3 * (t_j^2);          %速度公式
        a_t(j, i) = 2 * a2 + 6 * a3 * t_j;
    end
end

% movie_dir = 'UR5_Movie_Frames';  % 自定义帧保存目录
% if exist(movie_dir, 'dir')
%     rmdir(movie_dir, 's');  % 删除旧目录及所有内容（s：递归删除子文件）
% end

% %初始化仿真窗口(动画第一帧)
% figure('Position', [100, 100, 1000, 600]);  % 创建一个新的图形窗口，位置(100,100)，大小1000×600像素
% tool_robot.plot(q0);  % 调用机械臂模型的plot方法，绘制起始位姿（q0对应的姿态）
% title('UR5机械臂三次多项式插值轨迹规划仿真', 'FontSize', 14);  % 设置窗口标题
% xlabel('X (mm)'); ylabel('Y (mm)'); zlabel('Z (mm)');  % 设置坐标轴标签（单位mm，与机械臂模型一致）
% grid on;  % 显示网格线，方便观察坐标位置
% hold on;  % 保持当前图形，后续绘制不会覆盖已有内容（确保动画在同一窗口更新）

% for j = 1:length(t)  % 遍历所有时间点
%     q_current = q_t(j, :);  % 获取第j个时间点的关节角
%     tool_robot.plot(q_current, 'movie', 'on', 'fps', 1/dt);  % 用当前关节角更新机械臂姿态
%     drawnow;  % 强制刷新图形窗口
% end

% % 初始化绘图，并保留句柄
% figure('Position', [100, 100, 1000, 600]);
% h = tool_robot.plot(q_t(1,:), 'fps', 1/dt);  % 只绘制一次
% title('UR5机械臂三次多项式插值轨迹规划仿真', 'FontSize', 14);
% xlabel('X (mm)'); ylabel('Y (mm)'); zlabel('Z (mm)');
% grid on; hold on;
% 
% % 循环更新姿态（不再重新 plot）
% for j = 2:length(t)
%     tool_robot.animate(q_t(j,:));  % 仅更新关节角
%     drawnow;
% end
...
for i = 1:joints_dof  % 遍历每个关节
    ...
    % 子图1：关节角随时间变化曲线
    subplot(3, joints_dof, i);  % 划分3行6列的子图，当前绘制第i个（第一行）
    plot(t, q_t(:, i) * 180 / pi, 'LineWidth', 1.5);  % 绘制时间t vs 关节角（弧度转角度）
    xlabel('时间 t (s)');  % x轴：时间
...
sgtitle('UR5机械臂三次多项式插值轨迹规划结果', 'FontSize', 16);  % 总标题